...
\end{verbatim}
\begin{alltt}
\hlstd{data} \hlkwb{<-} \hlkwd{read.csv}\hlstd{(}\hlstr{"../datos/data_pvp.csv"}\hlstd{)}
\hlstd{metadata} \hlkwb{<-} \hlkwd{unique}\hlstd{(data[,} \hlkwd{colnames}\hlstd{(data)} \hlopt{%in%} \hlkwd{c}\hlstd{(}\hlstr{"ReplayID"}\hlstd{,}\hlstr{"Duration"}\hlstd{)])}
\hlcom{# clean_data <- data[,!(colnames(data) %in% c("ReplayID","Duration"))]}
\hlcom{# data_replay <- data[data$ReplayID==114, ]}
\hlcom{# data_replay <- data_replay[, !(colnames(data_replay) %in% c("ReplayID","Duration"))]}
\hlcom{# winner <- data_replay[1,"Winner"]}
\hlcom{# loser <- if(winner=='A') 'B' else 'A'}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{clean_data} \hlkwb{<-} \hlstd{data[data}\hlopt{$}\hlstd{ReplayID} \hlopt{!=} \hlnum{271}\hlstd{,]}
\hlstd{clean_metadata} \hlkwb{<-} \hlkwd{unique}\hlstd{(clean_data[,} \hlkwd{colnames}\hlstd{(data)} \hlopt{%in%} \hlkwd{c}\hlstd{(}\hlstr{"ReplayID"}\hlstd{,}\hlstr{"Duration"}\hlstd{)])}
\hlkwd{ggplot}\hlstd{(}\hlkwc{data}\hlstd{=clean_metadata)} \hlopt{+}
\hlkwd{geom_bar}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{x}\hlstd{=ReplayID,}\hlkwc{y}\hlstd{=Duration,}\hlkwc{fill}\hlstd{=Duration),} \hlkwc{stat}\hlstd{=}\hlstr{"identity"}\hlstd{)} \hlopt{+}
\hlkwd{geom_hline}\hlstd{(}\hlkwc{yintercept} \hlstd{=} \hlkwd{mean}\hlstd{(clean_metadata}\hlopt{$}\hlstd{Duration))}
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/ruido-1} 
\begin{kframe}\begin{alltt}
\hlkwd{print}\hlstd{(}\hlkwd{mean}\hlstd{(clean_metadata}\hlopt{$}\hlstd{Duration))}
\end{alltt}
\begin{verbatim}
## [1] 31582.96
\end{verbatim}
