...
en~\cite{DBLP:conf/aiide/StanescuHEGB13} y~\cite{DBLP:conf/cig/SynnaeveB11a}.

El caso más parecido al que se aborda en este trabajo
es~\cite{DBLP:conf/cosecivi/Sanchez-Ruiz15}, donde se intenta predecir el
...
de~\cite{DBLP:conf/flairs/RobertsonW14}, ya que no es un conjunto muy
...
\begin{itemize}
  ...
  permutaciones.
  \item Análisis Matemático: Razonamiento matemático deductivo e inductivo.
\end{itemize}
...
Las fuentes utilizadas son~\cite{Abu-Mostafa:2012:LD:2207825},
